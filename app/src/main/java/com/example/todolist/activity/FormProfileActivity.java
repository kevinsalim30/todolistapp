...
public class FormProfileActivity extends AppCompatActivity {
    ...
    public boolean validate(){
        if(editTextName.getText().toString().isEmpty()||!(radioFemale.isChecked()||radioMale.isChecked())){
            Toast.makeText(this,"Input semua form terlebih dahulu", Toast.LENGTH_LONG).show();
            return false;
        }else if(Integer.parseInt(textSeekbarAge.getText().toString())==0){
            Toast.makeText(this,"Age must be higher than 0", Toast.LENGTH_LONG).show();
            return false;
        }else{
            return true;
        }
